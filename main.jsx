...
export default function SqlScriptGenerator() {
  const [sheets, setSheets] = useState([]); // [{id, originalName, headers, rows}]
  ...
  const parseWorkbook = useCallback((arrayBuffer, name) => {
    try {
      ...
      const parsedSheets = workbook.SheetNames.map((sheetName, i) => {
        ...
        return { id: `sheet-${i}-${sheetName}`, originalName: sheetName, headers, rows };
      }).filter((s) => s.headers.length > 0);

      if (parsedSheets.length === 0) {
        setError("No usable sheets found — check the file has headers in row 1.");
        return;
      }
      ...
      parsedSheets.forEach((s) => {
        initialConfigs[s.id] = {
          ...
          keyColumns: new Set(),
          insertColumns: new Set(s.headers), // default: include every column
      ...
      setActiveId(parsedSheets[0].id);
      setFileName(name);
      setError("");
    } catch (e) {
      setError("Couldn't read that file. Make sure it's a valid .xlsx file.");
    }
  }, []);
      ...
      <div style={{ maxWidth: 1180, margin: "0 auto" }}>
        ...
        {sheets.length === 0 && (
          ...
          >
            ...
            <p style={{ margin: "0 0 18px", fontSize: 13, color: "#7C8AA5" }}>
              Every sheet becomes a table. First row must be column headers.
            </p>
            <label
              style={{
        ...
        {sheets.length > 0 && (
          <div style={{ marginTop: 22 }}>
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: "#7C8AA5" }}>
